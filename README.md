<h1 align="center">Hey 👋 I'm Arián</h1>
Rust developer. Interested in systems programming and low-level tooling.

## Currently working on

**[PeachDB](https://github.com/PeachGB/PeachDB)** — Async key-value database in Rust. File-backed storage engine with WAL, binary TCP protocol, and a custom type system — built on Tokio.

**[Regressor](https://github.com/PeachGB/Regressor)** — Machine learning library in Rust exposed to Python via PyO3

## Stack
[![My Skills](https://skillicons.dev/icons?i=rust,java,go,py,nextjs,aws)](https://skillicons.dev)

## Contact
Reach me at arianmateos@gmail.com
